...
// Shared 8-bot pipeline runner UI. Used by /chat (live run) and
// /feedback Panel 2 (test the updated prompts). Renders a job-input
// textarea, the run/cancel/new buttons, and the stream of bot cards.
// Does NOT render an outer page container — callers wrap with their own
// layout so headings and surrounding panels compose cleanly.
...
export interface PipelineRunnerProps {
  heading?: string;
  subheading?: string;
  /** When true, the Generate button is forced disabled regardless of input. */
  disabled?: boolean;
  /** Rendered next to the buttons when disabled=true. */
...
export default function PipelineRunner({
  ...
  subheading = "Paste the Upwork job post. The 8-bot pipeline will stream each bot's output as it completes.",
  disabled = false,
  disabledMessage = null,
  submitLabel = "Generate proposal",
  placeholder = "Paste the Upwork job post here...",
  textareaLabel = "Upwork job post",
}: PipelineRunnerProps) {
  ...
  const runPipeline = useCallback(async () => {
    ...
    try {
      ...
      console.warn("[pipeline/PipelineRunner] pipeline error:", message);
  ...
  const copyFinal = async () => {
    ...
    try {
      ...
      console.warn("[pipeline/PipelineRunner] clipboard write failed:", err);
    }
  };

  const charsLeft = MAX_JOB_INPUT_CHARS - jobInput.length;
  const runDisabled = jobInput.trim().length === 0 || disabled || ui.running;
...
function parseSseEvent(
  ...
  try {
    ...
    console.warn("[pipeline/parseSseEvent] malformed data:", err, dataStr);
    return null;
  }
}
